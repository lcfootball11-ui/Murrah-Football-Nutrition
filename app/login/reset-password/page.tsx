...
export default function ResetPasswordPage() {
  ...
  async function handleReset(e: React.FormEvent) {
    ...
    const { error: resetError } = await supabase.auth.resetPasswordForEmail(email, {
      redirectTo: `${window.location.origin}/login/reset-password/update`,
    })
          ...
          {sent ? (
            <div className="space-y-4">
              <div className="bg-green-500/10 border border-green-500/30 rounded-xl px-4 py-3">
                ...
                <p className="text-slate-400 text-xs mt-1">We sent a password reset link to <strong>{email}</strong></p>
              </div>
              <p className="text-slate-400 text-xs">Click the link in the email to create a new password.</p>
            </div>
          ) : (
